...
- 收藏家小屋会加入以下原版村庄房屋池：
  - 平原村庄：橡木、白陶瓦与平原村庄风格
  - 沙漠村庄：砂岩、平滑砂岩与沙漠村庄风格
  - 热带草原村庄：金合欢木、橙色陶瓦与热带草原村庄风格
  - 雪原村庄：云杉木、雪块与雪原村庄风格
  - 针叶林村庄：云杉木、苔石与针叶林村庄风格

收藏家小屋目前使用原版建筑方块搭建，并按村庄生物群系区分材质和装饰风格。每间小屋都会包含收藏家工作台、床、书架、室内装饰和一个专属收藏家战利品箱。屋内不会放置其它原版职业方块，避免村民在认领职业时误选；窗户使用完整玻璃块，室内也不再使用锁链提灯。

收藏家小屋会作为普通房屋加入原版村庄房屋池。当前权重已降低为 `1`，用于减少同一村庄重复生成多座收藏家小屋的概率；但原版 jigsaw 村庄会多次独立从房屋池抽取建筑，纯数据层不能严格保证“每个村庄最多一座”。如果后续需要绝对唯一，需要改为代码级村庄生成约束。

### 收藏家战利品箱

收藏家小屋中的箱子使用独立战利品表：`collection:chests/collector_house`。

- 固定生成 1 把刷子
- 随机生成若干考古相关物品
- 原版考古物品占主要权重，例如陶片、砖、金粒、煤、火药等
- 模组收藏品只以较低权重混入，避免玩家过早绕开遗迹探索
...
- `src/main/resources/data/collection/structure/`
  - 存放实际的 `.nbt` 遗迹模板。
- `src/main/resources/data/collection/structure/village/`
  - 存放五种生物群系风格的收藏家小屋 `.nbt` 模板。
- `src/main/resources/data/collection/loot_table/chests/collector_house.json`
  - 控制收藏家小屋专属战利品箱。
- `src/main/resources/data/collection/function/debug/place_collector_houses.mcfunction`
  - 调试用函数，可一次性放置五种生物群系收藏家小屋，便于快速验收外观。
...
不需要一个个寻找对应生物群系村庄。进入创造模式或开启作弊后，在一块开阔平地执行：

```mcfunction
/function collection:debug/place_collector_houses
```

这个调试函数会把平原、沙漠、热带草原、雪原、针叶林五种收藏家小屋按一排放出，方便同时检查门口楼梯、床朝向、窗户、火把、战利品箱和职业方块。
...
- `0.5.3`
  - 为平原、沙漠、热带草原、雪原、针叶林村庄制作差异化收藏家小屋。
  - 新增收藏家小屋专属战利品箱，固定包含刷子，并以原版考古物为主、少量混入模组收藏品。
  - 优化收藏家小屋内部：移除其它原版职业方块和室内锁链提灯，并将窗户改为完整玻璃块。
  - 修复收藏家小屋墙上火把没有贴附到墙体的问题。
  - 调整门前入口为对应材质楼梯，修正床朝向，并让床边外墙恢复为各小屋自身的墙体材料。
  - 修正门前入口楼梯朝向。
  - 将收藏家小屋在原版房屋池中的权重降为 1，并加入五种小屋一键放置调试函数。
- `0.5.2`
...
- 为收藏家小屋继续补充更精细的内部陈设、外部场景和最终美术资源。
